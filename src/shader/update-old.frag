...
void main() {
  ...
  if (length(col) == 0.0) {
    float xScale = 2.0;
    float yScale = 20.0;
    float r = 0.2+snoise2(vec2(pos.x*xScale+seed.x*xScale, pos.y*yScale+seed.y*yScale))/2.0+0.5;
    col = vec4(r, random(pos+2000.0*seed.y), random(pos+40.0*seed.z), 1.0);
  } else {

    float rng = random(pos);
    float rng2 = random(pos+2000.0);
    ...
    float m = modI(pos.x*dimension.x, 1.8)*modI(pos.y*dimension.y, 1.8);
